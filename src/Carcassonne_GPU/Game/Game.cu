...
DEVHOST Game::Game(int player_count, mb::u64 seed) : m_random_generator(seed),
...
DEVHOST const Board &Game::board() const noexcept {
   return m_board;
}

DEVHOST const Player &Game::current_player() const noexcept {
   return m_current_player;
}

DEVHOST mb::u8 Game::move_index() const noexcept {
   return m_move_index;
}

DEVHOST const TileSet &Game::tile_set() const noexcept {
   return m_tile_set;
}

DEVHOST bool Game::can_place(TileType tt) const noexcept {
   auto move_range = moves(tt);
   return move_range.begin() != move_range.end();
}

DEVHOST Move Game::new_move(Player p) noexcept {
...
//      fmt::print("m_move_index != move_index={}\n", m_move_index != move_index); // appears frequently
   ...
   return Move{p, tt, *this};
}

DEVHOST const std::vector<Figure> &Game::figures() const noexcept {
   return m_figures;
}

DEVHOST void Game::add_figure(Figure f) {
   --m_figure_count[static_cast<mb::size>(f.player)];
   m_figures.push_back(f);
}

DEVHOST void Game::apply_tile(int x, int y, TileType tt, mb::u8 rot) noexcept {
   ...
   std::vector<Group> tile_groups;
   tile_groups.reserve(8);
   ...
   for (int direction_id = 0; direction_id < 4; ++direction_id) {
      ...
      tile_groups.push_back(m_groups.group_of(edge));
   ...
   // ??? wcześniej tile_groups to był std::set, to czy musi być tu sortowanie?
   // std::sort(tile_groups.begin(), tile_groups.end());
   ...
   for (auto _x = x - 1; _x <= x + 1; _x++) {
      for (auto _y = y - 1; _y <= y + 1; _y++) {
         ...
         Player player;
         PlayerAssignment ignore;
         std::tie(ignore, player) = read_player_assignment(m_groups.assigment(make_edge(_x, _y, Direction::Middle)));
         on_monastery_completed(_x, _y, player);
      }
   }
}

DEVHOST void Game::on_structure_completed(Group g) {
   ...
   std::experimental::erase_if(m_figures,
...
DEVHOST bool Game::is_monastery_completed(int x, int y) noexcept {
   for (auto _x = x - 1; _x <= x + 1; _x++) {
      for (auto _y = y - 1; _y <= y + 1; _y++) {
...
DEVHOST void Game::on_monastery_completed(int x, int y, Player player) {
...
DEVHOST DEVHOST DEVHOST const ScoreBoard &Game::scores() const noexcept {
   return m_scores;
}

DEVHOST bool Game::is_town_field_connected(Edge town, Edge field) noexcept {
...
DEVHOST void Game::on_next_move(NextMoveCallback callback) noexcept {
   m_next_move_callbacks.push_back(callback);
}

DEVHOST std::vector<Direction> Game::figure_placements(int x, int y) const noexcept {
   ...
   while (connections != Connection::None) {
      ...
      std::experimental::erase(result, a);
   }

   return result;
}

DEVHOST bool Game::can_place_figure(int x, int y, Direction d) const {
...
DEVHOST void Game::assign_final_points() noexcept {
...
DEVHOST mb::u8 Game::player_figure_count(Player p) const noexcept {
   return m_figure_count[static_cast<mb::size>(p)];
}

DEVHOST void Game::update(double dt) noexcept {
   if (m_tour_finished) {
...
DEVHOST void Game::notify_tour_finished(FullMove full_move) noexcept {
   m_tour_finished = true;
   m_performed_move = full_move;
}

DEVHOST ecuda::unique_ptr<Game> Game::clone() const noexcept {
   return ecuda::unique_ptr<Game>{new Game{*this}};
}

DEVHOST static int score_monastery(const Board &board, int x, int y, int moastery_score) {
   int score = 0;
...
DEVHOST int Game::score_grass(Player player, Edge edge, const Parameters &params) const noexcept {
...
DEVHOST static bool owns_edges(bool free_a, bool owns_a, bool free_b, bool owns_b) {
   if (!free_a && !owns_a)
      return false;
   if (!free_b && !owns_b)
      return false;
...
DEVHOST int Game::score_tile(Player player, const Tile &tile, TileMove move, Direction target_direction, const Parameters &params) const noexcept {
...
DEVHOST int Game::score_direction(Player player, TileType tile_type, TileMove move, Direction direction, const Parameters &params) const noexcept {
...
DEVHOST std::pair<Direction, int> Game::move_score(Player player, TileType tile_type, TileMove move, const Parameters &params) const noexcept {
   Direction best_dir{};
...
template <typename E>
DEVHOST constexpr typename std::underlying_type<E>::type to_underlying(E e) noexcept {
    return static_cast<typename std::underlying_type<E>::type>(e);
}

template <typename T, typename U>
DEVHOST inline void flags_to_states(const T flags, const int neurons_to_set, U output_it) noexcept {
...
template <typename T, typename U>
DEVHOST inline void vector_to_states(const T vector, const size_t neurons_to_set, U& output_it) noexcept {
   std::vector<bool> states(neurons_to_set, false);
...
template <typename U>
DEVHOST inline void tile_to_caffe(const Tile& tile, const U it) noexcept {
   auto output_it = it;
...
DEVHOST inline void figures_to_caffe(const F& figures, int x, int y, U output_edges_it, mb::size player_count, const U it) noexcept {
   auto output_it = it;
...
DEVHOST void Game::board_to_caffe_X(std::vector<float> &output) const {
...
DEVHOST static int pc_find(const std::array<int, g_directions.size()> &dirs, int value) {
...
DEVHOST static void pc_join(std::array<int, g_directions.size()> &dirs, std::array<bool, g_directions.size()> &free, int a, int b) {
...
DEVHOST bool Game::can_place_tile_and_figure(int x, int y, mb::u8 rot, TileType tile_type, Direction d) const {
