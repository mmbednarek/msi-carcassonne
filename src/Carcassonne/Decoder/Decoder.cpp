...
static float fill_allowed_vec(const std::unique_ptr<IGame> &game, TileType tile, std::vector<bool> &allowed_moves, const std::span<float> blob) {
...
FullMove decode_move(const std::unique_ptr<IGame> &game, TileType tile, std::vector<bool> &allowed_moves, const std::span<float> blob, float prob) {
   using mb::size, mb::u8;
