#include <Carcassonne/Game/Board_GPU.h>

namespace carcassonne::game {

int Board_GPU::min_x() const noexcept {
   return m_min_x;
}

int Board_GPU::min_y() const noexcept {
   return m_min_y;
}

int Board_GPU::max_x() const noexcept {
   return m_max_x;
}

int Board_GPU::max_y() const noexcept {
   return m_max_y;
}

TilePlacement Board_GPU::tile_at(int x, int y) const noexcept {
   if (x < 0 || y < 0 || x >= g_board_width || y >= g_board_height)
      return TilePlacement{.type = 0};
   return m_board.get(x, y);
}

void Board_GPU::set_tile(int x, int y, TileType t, mb::u8 rotation) noexcept {
   if (x < 0 || y < 0 || x >= g_board_width || y >= g_board_height)
      return;
...
bool Board_GPU::can_place_at(int x, int y, TileType t, mb::u8 rotation) const noexcept {
   if (x < 0 || y < 0 || x >= g_board_width || y >= g_board_height)
      return false;
...
TilePlacement Board_GPU::tile_at(TilePosition pos) const noexcept {
